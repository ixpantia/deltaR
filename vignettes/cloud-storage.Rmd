---
title: "Using deltalakeR with Cloud Storage"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using deltalakeR with Cloud Storage}
...
deltalakeR provides native support for reading and writing Delta tables to cloud storage services, including:
 - **Amazon S3** (Linux/macOS only)
- **Google Cloud Storage (GCS)**
- **Azure Blob Storage / Azure Data Lake Storage (ADLS)**

This guide covers how to configure and use each cloud storage provider with deltalakeR.

> **Platform Note:** S3 support is currently only available on Linux and macOS due to compilation issues on Windows. Windows users should use Google Cloud Storage or Azure Blob Storage for cloud-based Delta tables.

## General Concepts

### Storage Options

All cloud storage operations in deltalakeR use the `storage_options` parameter to pass authentication credentials and configuration. This is a named list where keys and values depend on the cloud provider.
...
> **Platform Availability:** Linux and macOS only. S3 support is not available on Windows due to compilation issues with the underlying dependencies.

### Authentication Options

deltalakeR supports multiple authentication methods for S3:
...
When running on AWS infrastructure with an IAM role attached, you typically only need to specify the region:
...
deltalakeR works with S3-compatible storage services like MinIO, LocalStack, or Ceph:
...
When running on GCP infrastructure (Compute Engine, Cloud Run, GKE) or with `gcloud` configured locally:
...
- **AWS**: Attach IAM roles to EC2 instances, Lambda functions, or ECS tasks
- **GCP**: Use service accounts attached to Compute Engine VMs or Cloud Run services
...
- Check IAM permissions include required actions (GetObject, PutObject, etc.)
...
options(deltalakeR.verbose = TRUE)
...
deltalakeR's cloud storage support is powered by the [object_store](https://github.com/apache/arrow-rs-object-store) crate from the Apache Arrow project and the [delta-rs](https://github.com/delta-io/delta-rs) library. We thank the maintainers and contributors of these projects for their excellent work.
