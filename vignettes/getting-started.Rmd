---
title: "Getting Started with deltalakeR"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with deltalakeR}
...
## Introduction
deltalakeR provides an R interface to [Delta Lake](https://delta.io/), the open-source storage layer that brings ACID transactions to data lakes. Built on the high-performance [delta-rs](https://github.com/delta-io/delta-rs) Rust library, deltalakeR enables you to read and write Delta tables directly from R with minimal overhead.
...
deltalakeR supports four write modes:
...
deltalakeR delegates reading to other libraries like arrow, polars, or duckdb. Use `get_files()` to get the list of Parquet files and pass them to your preferred library:
...
- Read the [Cloud Storage Guide](cloud-storage.html) to learn about using deltalakeR with S3, GCS, and Azure
...
deltalakeR is built on the excellent [delta-rs](https://github.com/delta-io/delta-rs) Rust library. We are grateful to the delta-rs maintainers and the broader Delta Lake community for their work.
